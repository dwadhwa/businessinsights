...
var getOrders = function() {
  ...
  var url = 'https://api.yaas.io/hybris/order/v1/businessinsights/salesorders'
  req.onreadystatechange = function() {
    if (this.readyState === 4) {
      if (this.status === 200) {
        var response = req.responseText;
        var innerdiv = document.getElementById('responsediv');
        innerdiv.innerHTML = JSON.parse(response)
        var _jsonObj = JSON.parse(response);
        var data = [
    {"Product": "T-shirt", "name":"T-shirt", "Quantity": 15},
    {"Product": "Shoes", "name":"Shoes", "Quantity": 10},
    {"Product": "Towel", "name":"Towel", "Quantity": 20},
    {"Product": "Hats", "name":"Hats", "Quantity": 32}
  ]
